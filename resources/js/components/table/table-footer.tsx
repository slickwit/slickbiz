...
export default function TableCustomFooter<T>({ table }: TableCustomFooterProps<T>) {
    const { getCanPreviousPage, getCanNextPage, getState, setPageIndex, setPageSize, firstPage, previousPage, nextPage, lastPage, toggleDensity } =
        table;
            ...
            <div className="mb-2 flex w-full flex-wrap items-center justify-end max-lg:flex-col-reverse max-lg:items-end sm:mb-0 md:space-x-4">
                <div className="block h-8 max-sm:absolute max-sm:right-2.5 max-sm:bottom-2.5 sm:h-10">
                    <div className="flex items-center space-x-1">
                        <Label htmlFor="row-per-page-select" className="text-common text-sm">
                            Rows per page:
                        </Label>
                    ...
                    {paginationRange && paginationRange.length > 0 ? (
                        paginationRange?.map((paginate, idx) => (
                            <IconButton
                                ...
                                className="data-[active=true]:text-common data-[active=true]:bg-common/12 text-sm font-medium data-[active=true]:font-bold data-[active=true]:opacity-100"
                            >
                                {paginate !== null ? paginate : <Ellipsis width={16} height={16} className="!size-4" />}
                            </IconButton>
                        ))
                    ) : (
                        <IconButton transitionOff className="text-common bg-common/12 pointer-events-none text-sm font-bold opacity-100">
                            1
                        </IconButton>
                    )}
