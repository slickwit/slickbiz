...
const Autocomplete = ({
    ...
    const handleInputChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!asynFilterFunction) {
            const filteredOptions = options.filter((opt) => opt.value.toLocaleLowerCase().includes(e.currentTarget.value.trim().toLocaleLowerCase()));
    ...
    return (
        <Popover open={focused.value}>
            <PopoverTrigger asChild>
                <div className={cn(triggerClass, 'group/input relative')}>
                    ...
                    <IconButton
                        size="xs"
                        className={cn(
                            'invisible absolute top-1/2 right-2 -translate-y-1/2 transform select-none group-hover/input:visible',
                            !inputValue && 'hidden',
                        )}
                        onClick={handleReset}
                    >
            ...
            <PopoverContent
                ...
                className={cn(contentClass, 'flex max-h-[300px] w-[var(--radix-popper-anchor-width)] flex-col items-start overflow-y-auto p-0')}
            >
                        ...
                        <>
                            ...
                            {canAdd && (
                                <li
                                    ...
                                    className={cn(
                                        'w-full cursor-pointer rounded-none px-2 py-1.5 text-start hover:bg-accent',
                                        focusedOption === optionList.length && 'bg-accent ring-1 ring-primary',
                                    )}
                                >
                                    Create &quot;{inputValue}&quot;
                                </li>
                            ...
                            {notFound && (
                                <li aria-label="not found" className="w-full rounded-none px-2 py-1.5 text-start">
                                    Not Found
                                </li>
                            )}
                        </>
                    ) : (
                        <li aria-label="loading" className="w-full rounded-none px-2 py-1.5 text-center">
                            {loadingLabel}
                        </li>
                    )}
                </ul>
