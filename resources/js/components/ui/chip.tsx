...
function Chip({
    ...
    return (
        <div className={cn(chipVariants({ variant, color, className, size }))} data-clickable={!!onClick} onClick={onClick}>
            ...
            <span {...labelProps} className={cn('label', labelProps?.className)}>
                {label}
            </span>
            {!!deletable && (
                <div className={cn(deletableVariants({ variant, color }), 'deletable')} data-variant={variant} data-color={color} onClick={onDelete}>
